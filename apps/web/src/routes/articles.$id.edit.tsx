...
function EditArticle() {
  ...
  useEffect(() => {
    api
      ...
      .catch((e) => setLoadError(e instanceof ApiError ? e.message : "not found"))
  }, [id])

  // Memoize the initial JSON so the Lexical composer isn't re-initialized on every render.
  const initialStateJson = useMemo(() => article?.bodyJson ?? null, [article?.id])
    ...
    <main className="">
      <header className="flex items-center justify-between border-b border-border px-4 py-3">
        <h1 className="text-sm font-semibold text-muted-foreground">
          {article.status === "draft" ? "draft" : "editing"} · {article.readingMinutes} min read
        </h1>
